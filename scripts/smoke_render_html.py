"""Smoke test: render a hand-built TailoredAssets through HTML renderer.

Produces scripts/smoke_resume_html.pdf so we can
eyeball the output before touching S3.

Run: python scripts/smoke_render_html.py
"""
import sys
...
OUT_PDF = Path("scripts/smoke_resume_html.pdf")
...
print(f"Rendering tailored PDF via HTML for: {assets.job_title_used} @ {assets.company_name_used}")
try:
    pdf_bytes = render_resume_pdf(assets, profile)
    ...
    assert len(text) > 2500, "extracted PDF text is unexpectedly short"
    assert "**" not in text, "markdown bold markers leaked into PDF text"
    for required in (
        "Rahul Bajaj",
        "linkedin.com/in/rahulbajaj",
        "github.com/bajajankur21",
        "WORK EXPERIENCE",
        "PROJECTS",
        "EDUCATION",
...
print("\nSmoke test PASSED — open the PDF to eyeball formatting.")
