...
python test_pipeline.py --gemini-only                 # no Codex/AWS cost — gemini for 0a+1, mock 2
...
There is no unit-test framework — `test_pipeline.py` is an end-to-end driver with per-agent mode switches defined in `tests/fixtures.py` (mocks) and `tests/gemini_overrides.py` (Gemini swaps for Codex agents).
...
- **Agent 0A (profiler)** — Codex Sonnet via **tool-use** (`produce_tailored_resume`-style forced tool call). Runs once per pipeline invocation.
...
- **Agent 1 (tailor)** — two-tier routing in `main.py`:
  - **Top `TOP_TIER_CLAUDE_COUNT` jobs** (default 6): Codex Sonnet with **prompt caching on the master resume** (`cache_control: ephemeral`). Cached once, reused across every Codex call in the run (5-minute TTL). Tool-use for structured `TailoredAssets` output.
...
`tests/gemini_overrides.py` provides drop-in Gemini replacements **only for the Codex agents** (0A and 1). The profiler override (`build_candidate_profile_gemini`) still uses `gemini-2.5-flash-lite` with native `response_schema` (Gemini models do support JSON mode). The tailor override re-exports `run_tailor_gemini` directly from `agents/agent_1.py`, which now points at Gemma 4 31B with prompt-driven JSON — keep that in mind if you're debugging `--gemini-only` runs: Agent 0A's output is schema-enforced, Agent 1's is not.

When modifying Agent 0A or 1 prompts or output schemas, update **both** the Codex path and the Gemini override. For Agent 1 specifically, also update the `_GEMMA_JSON_TEMPLATE` literal and the `R1–R12` hard rules in `run_tailor_gemini` — they are the only thing keeping Gemma's output valid.
...
- `TOP_TIER_CLAUDE_COUNT` (default 6) — how many top-scored jobs use Codex before the pipeline falls through to Gemma.
